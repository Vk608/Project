...
namespace FABBatchValidator.Excel
{
    ...
    public class ExcelInputReader
    {
        ...
        public List<BiblioRecord> ReadRecords()
        {
            try
            {
                if (!File.Exists(_fileConfig.InputFilePath))
                    throw new ExcelInputException($"Input file not found: {_fileConfig.InputFilePath}");

                using (var package = new ExcelPackage(new FileInfo(_fileConfig.InputFilePath)))
                {
